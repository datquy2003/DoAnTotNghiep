...
const TooltipMoney = ({ active, payload, label }) => {
  ...
  return (
    <div className="px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg shadow-sm">
      ...
      <div className="text-gray-700">Doanh thu: {money(val)}</div>
    </div>
  );
};
...
export default function RevenueStats() {
  ...
  const load = async (isRefresh = false) => {
    ...
    try {
      ...
      console.error("Lỗi load revenue report:", err);
      toast.error(
  ...
  const pieData = useMemo(
    () => (Array.isArray(data?.pie) ? data.pie : []),
    [data]
  );
  ...
  return (
    <div className="w-full">
      <div className="text-2xl font-bold text-gray-900">Doanh thu</div>
        ...
        <div className="grid grid-cols-1 gap-3 mt-4 md:grid-cols-2">
          <div className="p-4 border border-gray-100 rounded-xl bg-gray-50">
            ...
            <div className="mt-1 text-2xl font-bold text-gray-900">
              {loading ? "…" : money(data?.totalRevenue || 0)}
            </div>
          </div>
          <div className="p-4 border border-gray-100 rounded-xl bg-gray-50">
